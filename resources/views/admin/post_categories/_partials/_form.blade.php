    ...
    <div class="tab-pane p-3 active" id="info" role="tabpanel">
        <div class="row mb-4">
            ...
            <div class="col-sm-6 text-right">
                <a href="{{ route('post-categories.index') }}" class="btn btn-primary waves-effect waves-light float-end">
                    <i class="mdi mdi-format-list-bulleted pe-2"></i> Zoznam kategórií
                </a>
            </div>
        </div>
                        ...
                        <div class="tab-pane p-3 {{ $loop->first ? 'active' : '' }}" id="{{ $key }}" role="tabpanel">
                            <div class="row mb-3">
                                <div class="col-sm-6">
                                    ...
                                    <input name="name_{{ $key }}" type="text" value="{{ old("name_$key", isset($post_category) ? $post_category->{"name_$key"} : '') }}" class="form-control parse-slug{{ $errors->has("name_$key") ? 'is-invalid' : '' }}" data-slug-selector='input[name="slug_{{ $key }}"]'>
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <input name="slug_{{ $key }}" type="text" value="{{ old("slug_$key", isset($post_category) ? $post_category->{"slug_$key"} : '') }}" class="form-control {{ $errors->has("slug_$key") ? 'is-invalid' : '' }}">
